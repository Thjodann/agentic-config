...
> [!TIP]
> You are done when `agentic status` reports no Blocking items and no generated
...
unless I explicitly ask. If status reports Blocking conflicts, stop and explain them.
```

## 🧭 Which Setup Should I Use?

Most people should start with `agentic setup .`. Use the other paths when you are
...
| I want my team to share this config | `agentic setup .` | Creates a committed `.ai/` folder for shared AI setup. |
| I want to try this privately | `agentic setup --stealth .` | Creates local-only Agentic Config files without tracked Git changes. |
| This repo already has Cursor, Claude, Codex, or Windsurf files | `agentic status` then `agentic import --all` | Shows what can be imported, then imports unambiguous assets. |
| I just cloned a repo that already uses Agentic Config | `agentic setup .` | Regenerates local AI tool files from the shared source. |
...
> wording, so `import --all` promotes one surface per asset into `.ai/` and then
...
- **`import` only scans standard IDE asset folders.** The bundled skills under
  paths like `.cursor/<name>/SKILL.md` (instead of `.cursor/skills/<name>/SKILL.md`)
  are intentionally left native; Agentic Config does not import skills from
...
If the report looks safe, import everything Agentic Config can import automatically:
...
`agentic import --all` imports every unambiguous native asset in one pass. If two
...
If `status` still reports conflicts or same-name files with different content,
...
for you. Once the source you imported lives in `.ai/`, `agentic sync` refreshes the
matching native files in place, so the import → sync → status loop completes.
...
| Show health, blockers, and the next step | `agentic status` |
| Import existing native files | `agentic import --all` |
| Import one native file | `agentic import <path>` |
| Import exact duplicate groups | `agentic import --exact-duplicates` |
| Set up or regenerate files after cloning | `agentic setup .` |
...
/agentic-config import this Cursor rule into the shared config
/agentic-config import exact duplicate skills across the repo
/agentic-config demote the generated Cursor commit command
/agentic-config set up this clone
```
...
## 🩺 What Do Status, Doctor, And Check Mean?

`agentic status` is the everyday command. It summarizes health in priority order:

```text
Blocking -> Needs update -> Recommended cleanup -> Optional / informational -> Next step
```
...
Use `agentic status --verbose` when you want the full doctor-style details with
...
| `agentic status` reports stale output | Run `agentic sync`, then run `agentic status` again. |
| `agentic status` reports native-only files | Import them with `agentic import <path>`, use `agentic import --all`, or leave them native. |
...
- Run `agentic status` before committing.
- Use `agentic import` when adopting existing tool files or reconciling exact duplicates.
