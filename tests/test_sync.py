...
class SyncRepo(unittest.TestCase):
    def setUp(self):
        ...
        # Keep the fake HOME outside the repo tree so user-level (`~/...`) global
        # assets are classified as global, not repo-relative. Nesting HOME inside
        # the repo only happens to work where /tmp is symlinked (e.g. macOS).
        self.home = tempfile.mkdtemp(prefix="agentic-kit-home-")
...
""")
    ...
    def test_adopt_all_continues_past_canonical_conflict(self):
        # Same logical skill on three surfaces, but one diverges in content.
        # A single conflict must not abort adoption of the rest.
        self.add_native_skill(".agents/skills", "shared", body="Body A.")
        ...
        # First surface won canonical; the later, unrelated asset still adopted.
    ...
    def test_adopt_all_skips_surface_already_represented(self):
        ...
        self.add_native_skill(".cursor/skills", "dup", body="Identical body.")

        result = self.run_sync("adopt", "--all")
        self.assertIn("adopted 1 native assets", result.stdout)
        self.assertIn(
            "skip .cursor/skills/dup/SKILL.md: already represented in .ai/",
            result.stdout)
        self.assertTrue(os.path.exists(self.path(".ai", "skills", "dup", "SKILL.md")))

    def test_adopt_then_sync_refreshes_adopted_native_in_place(self):
        write(self.path(".cursor", "rules", "api.mdc"), """---
description: API route conventions.
globs: app/api/**/*.py
alwaysApply: false
...
""")
        self.run_sync("adopt", "cursor", ".cursor/rules/api.mdc")
        # The documented round-trip: adopt then sync must succeed in place.
        result = self.run_sync()
        self.assertIn("Synced 1 canonical assets", result.stdout)
        native = read(self.path(".cursor", "rules", "api.mdc"))
        self.assertIn("AUTOGENERATED from", native)
        self.assertIn("Use service boundaries.", native)

        check = self.run_sync("check")
        self.assertIn("In sync", check.stdout)

    def test_adopt_all_then_sync_completes_for_nonconflicting_repo(self):
        write(self.path(".cursor", "rules", "api.mdc"), """---
description: API route conventions.
---
...
Use service boundaries.
""")
        write(self.path(".claude", "agents", "reviewer.md"), """---
name: reviewer
description: Reviews code for correctness.
---
...
Review like an owner.
""")
        self.run_sync("adopt", "--all")
        result = self.run_sync()
        self.assertIn("Synced 2 canonical assets", result.stdout)
        check = self.run_sync("check")
        self.assertIn("In sync", check.stdout)

    def test_sync_still_refuses_divergent_markerless_native_after_adopt(self):
        # The represented-overwrite relaxation must not weaken the guard for a
        # genuinely divergent markerless native sharing a generated path.
        self.add_native_skill(".agents/skills", "fixture", body="Canonical body.")
        self.run_sync("adopt", "codex", ".agents/skills/fixture/SKILL.md")
        write(self.path(".cursor", "skills", "fixture", "SKILL.md"), """---
name: fixture
description: Divergent human copy.
---

Different human body that must be protected.
""")
        ...
        self.assertIn(
            "Different human body that must be protected.",
            read(self.path(".cursor", "skills", "fixture", "SKILL.md")))
